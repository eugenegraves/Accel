...
function RaceDistanceCard({
  ...
  const deltaColor = deltaFromPr <= 0 ? 'text-emerald-400' : 'text-red-400';
      ...
      {expanded && (
        <div className="mt-4 pt-4 border-t border-slate-700 space-y-2">
          <div className="flex justify-between text-sm">
            <span className="text-slate-400">PR Meet</span>
            <span className="text-slate-300">{prMeetName}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-slate-400">PR Date</span>
            <span className="text-slate-300">{prDate}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-slate-400">Season Best</span>
    ...
    <div className="flex-1 flex flex-col">
      ...
      <header className="bg-slate-800 border-b border-slate-700 px-4 py-3 pt-safe-top">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-semibold text-slate-100">Meet Review</h1>
        </div>
      </header>
      ...
      <main className="flex-1 overflow-auto p-4 pb-safe-bottom">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12 text-center">
            <div className="text-slate-400 mb-4">No race data yet</div>
